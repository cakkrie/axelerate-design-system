...
- **Touch targets:** every default size clears 44×44px — `Button` md/lg, `IconButton` md/lg, `Input` / `Textarea`, `Select`. The `sm` sizes (`Button` 32px, `IconButton` 28px) are deliberately below it and are **not for touch**: reach for them only in dense, pointer-driven contexts, never as the size a thumb has to hit.
...
- `assets/icons/` — 42 line icons: Doodle Icons plus `gift`, drawn here in the same idiom (hand-drawn, tintable via mask). `ls assets/icons | wc -l` is the count; this line has been wrong before
