...
- **Type:** **DynaPuff** (`--font-title`: screen titles only — weight 600, UPPERCASE, +0.01em, `--text-xl` (22px), or `--text-2xl` for a hero screen. One per screen section, never inside a component) / Bricolage Grotesque (display, 700–800, tracking −0.02…−0.035em) / Instrument Sans (body/UI) / **Gabarito** (`--font-label`: stats, stamps, caps labels at +0.07em, weight 500 for captions and 700 for big figures, `tabular-nums` on anything that counts) / Lacquer (margin annotations, 18–34px, ±2–3° tilt — never UI labels). Scale 10→80px in **rem** (`--text-3xs…6xl`), so a reader's own browser font size and 200% zoom both work. The app ladder is `3xs`·`xs`·`sm`·`lg`·`xl` = 10·12·15·18·22 at ratios 1.20–1.25 — five rungs, with weight, family and colour carrying the rest of the hierarchy. `--text-md` (16) is the wide-measure prose alternate to `--text-sm` (15), **not** a step above it: never use the two together as a hierarchy pair. Body 15px at 1.55. No monospace anywhere — the technical voice comes from Space Grotesk's letterforms and tracking, not from fixed pitch.
